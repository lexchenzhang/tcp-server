...
func DoConnectionBegin(conn ziface.IConnection) {
	...
	conn.SetProperty("Str", "test_str")
	conn.SetProperty("Int", 512)
}

func DoConnectionClose(conn ziface.IConnection) {
	fmt.Println("user's on conn stop....")
	v_str, _ := conn.GetProperty("Str")
	v_int, _ := conn.GetProperty("Int")
	fmt.Println(v_str, " ", v_int)
}

func main() {
	s := znet.NewServer("[server v0.10]")
	// register hook func
